...
export default function SentinelQA() {
  ...
  useEffect(() => {
    ...
    // Real-time subscription to see results pop up live
    const channel = supabase.channel('schema-db-changes')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'test_results' }, () => fetchData())
      .subscribe();

    return () => { supabase.removeChannel(channel) };
  }, []);

  const filteredData = data.filter(res => res.test_runs?.environments?.name === activeEnv);
  ...
  return (
    <main className="min-h-screen bg-[#0F172A] text-slate-200 font-sans selection:bg-amber-500/30">
      {/* SECTION 1: TOP BRANDING BAR */}
      <nav className="bg-[#0F172A] px-8 py-5 flex justify-between items-center border-b border-slate-800 shadow-2xl sticky top-0 z-50">
        <div className="flex items-center gap-5">
          <div className="relative">
            <div className="bg-amber-500 p-2 rounded-xl text-[#0F172A] font-black text-2xl w-12 h-12 flex items-center justify-center shadow-[0_0_20px_rgba(245,158,11,0.3)]">
              S
            </div>
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter text-white leading-none uppercase">
              Sentinel <span className="text-amber-500">QA</span>
            </h1>
            <p className="text-[10px] text-slate-500 uppercase tracking-[0.3em] mt-1 font-bold">W2S Solutions • Watchtower</p>
          </div>
        </div>
        <button className="bg-amber-600 hover:bg-amber-500 text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-amber-900/20 active:scale-95">
          Export Report
        </button>
      </nav>
      ...
      <div className="bg-[#111827] px-8 border-b border-slate-800/50">
        <div className="flex gap-12">
          {['Dashboard', 'Execution Logs', 'Historical Trends', 'Settings'].map((section) => (
            <button
              key={section}
      ...
      {activeSection === 'Dashboard' ? (
        <div className="p-8 max-w-7xl mx-auto animate-in fade-in duration-500">
          ...
          <div className="bg-[#1E293B]/40 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl backdrop-blur-sm">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Recent Executions: {activeEnv}</h2>
            </div>
            <div className="divide-y divide-slate-800/50">
              {loading ? (
                <div className="p-20 text-center text-slate-600 font-mono text-sm uppercase tracking-widest">Scanning Database...</div>
        ...
        <div className="flex flex-col items-center justify-center h-[60vh] text-slate-700 opacity-50">
          <p className="text-2xl font-black uppercase tracking-[0.5em]">{activeSection}</p>
          <p className="text-xs mt-4 font-mono">Module integration pending at W2S Solutions</p>
        </div>
      )}
    </main>
  );
