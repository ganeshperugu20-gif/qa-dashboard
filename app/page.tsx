...
export default function SentinelQA() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      // Fixed query with !env_id to resolve relationship ambiguity
      ...
      if (error) {
        ...
        console.log("Telemetry Received:", results);
        setData(results || []);
      }
      setLoading(false);
    }
  ...
  // Filter logic: Handles case sensitivity and potential whitespace
  ...
  return (
    <main className="min-h-screen bg-[#0F172A] text-slate-200 font-sans selection:bg-amber-500/30">
      ...
      <nav className="bg-[#0F172A] px-8 py-5 flex justify-between items-center border-b border-slate-800 shadow-2xl sticky top-0 z-50">
        ...
        <button className="bg-amber-600 hover:bg-amber-500 text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-amber-900/20">
          Export Report
        </button>
      </nav>
      ...
      <div className="p-8 max-w-7xl mx-auto">
        ...
        <div className="bg-[#1E293B]/40 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl backdrop-blur-sm">
          ...
          <div className="divide-y divide-slate-800/50 min-h-[300px]">
            {loading ? (
              <div className="p-20 text-center text-slate-600 font-mono text-sm uppercase tracking-widest animate-pulse">Establishing Telemetry...</div>
              ...
              filteredData.map((res: any) => (
                <div key={res.id} className="p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all group">
                  <div className="flex gap-6 items-center">
                    <div className={`w-1 h-12 rounded-full ${res.status === 'passed' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.3)]'}`} />
                    <div>
              ...
              <div className="p-20 text-center text-slate-700 font-bold uppercase tracking-widest italic">
                No telemetry data detected for {activeEnv}
              </div>
            )}
          </div>
        </div>
